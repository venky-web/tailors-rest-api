...
class PaymentListCreateView(ListCreateAPIView):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if customer_id:
            payments = self.get_queryset().filter(user_id=customer_id, is_deleted=False).order_by("-updated_on")
        else:
            payments = self.get_queryset().filter(is_deleted=False).order_by("-updated_on")
...
class PaymentDetailView(RetrieveUpdateDestroyAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        serializer.save(request_user=request.user, updated_on=f.get_current_time())
        return Response(serializer.data, status=status.HTTP_200_OK)
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        serializer = self.get_serializer(payment, data=modified_data)
        serializer.is_valid()
        serializer.save(request_user=request.user, updated_on=f.get_current_time())
        return Response(serializer.data, status=status.HTTP_200_OK)
